...
export default function EditProgramPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    const supabase = createClient();
    const { error } = await supabase.from("programs").update({ title: form.title, description: form.description || null, tag: form.tag, icon: form.icon, color: form.color, is_featured: form.is_featured }).eq("id", id);
