              ...
              {submissions.map((sub) => (
                <button key={sub.id} onClick={() => { setSelectedId(sub.id); if (!sub.is_read) toggleRead(sub.id, false); }}
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full shrink-0 ${sub.is_read ? "bg-gray-600" : "bg-cyan-400"}`} />
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => toggleRead(selected.id, selected.is_read)}
                    className="p-2 text-gray-400 hover:text-cyan-400 hover:bg-white/5 rounded-lg transition-all cursor-pointer" title={selected.is_read ? "Mark as unread" : "Mark as read"}>
